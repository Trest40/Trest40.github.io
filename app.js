...
(() => {
  ...
  function selectStation(id) {
    ...
    detail.innerHTML = `
      ...
      <div style="margin-top:14px"><span class="status-pill s-${s.status}"><i></i>${t.statusLevel[s.status]} ${t.levelWord}</span></div>
      <div class="d-readouts">
        <div class="ro"><div class="v">${hasLevel ? Math.round(s.level) : '—'} <small>${t.cm}</small></div><div class="k">${t.mean}</div></div>
        <div class="ro"><div class="v">${hasLevel ? Math.round(s.level_max) : '—'} <small>${t.cm}</small></div><div class="k">${t.max}</div></div>
      </div>
    ...
    if (s.anchor) {
      ...
      link.href = '#hydrograph';
      link.className = 'btn btn-primary';
      link.style.cssText = 'margin-top:16px;width:100%;justify-content:center;font-size:14px;padding:11px';
      link.textContent = t.fullHydro;
